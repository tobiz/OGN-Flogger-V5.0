    ...
    try:
        ...
        if comment != "": 
            ...
            if test_base(via, settings):
                print( "via found and on list of base stations: ", via, " From: ", parse_dict["from"])
            ...
            print("via key not present in Flarm Base parse dictionary:", via)
            return False
        else:
            print("APRS returned comment field: Blank: ")
    except:
