...
# 20220120            Changed int(settings.FLOGGER_QNH) to float(settings.FLOGGER_QNH) as original through an error as string value 
#                     was not an int format value ie was format nnn.nnn
#
...
def process_log (cursor, db, settings):
    ...
    print("+++++++Phase 0, check for Not Landed flights and zero duration flights+++++++")
    sar = "SAR"
    #zt = datetime.time(0, 0, 0)  # datetime object for 00: 00: 00 hrs: min: sec                                               # SAR seems to stand for: Search And Rescue - See KTrax!
    #cursor.execute('''SELECT sdate, stime, edate, etime, duration, src_callsign, max_altitude, speed, registration, id FROM flight_log2 WHERE edate=? AND etime=?''', (sar, sar,))
    #cursor.execute('''SELECT sdate, stime, edate, etime, duration, src_callsign, max_altitude, speed, registration, id FROM flight_log2 ''')
    #allrows = cursor.fetchall()
    #for arow in allrows:
    #    print "Check Flight", "registration: ", arow[8], " stime: ", arow[1], " edate: ", arow[2] 
        #
        # 20210901 Check whether duration is 00 minutes, if so ignore
        #
        #if arow[2] == sar:
        #    print "SAR flight"
        #    #arow[4] = time.strptime("00: 00: 00", "%H: %M: %S")     # Set duration to 00: 00: 00. arow[4] gets used on INSERT later
        #    #duration = time.strptime("00: 00: 00", "%H: %M: %S")
        #else:    
        #    time_str = arow[4].replace("h", "")
        #    time_str = time_str.replace("m", "")
        #    time_str = time_str.replace("s", "")
        #    print "Duration of non-SAR flight in flight_log2: ", time_str
            #duration = time.strptime(time_str, "%H: %M: %S")
        #    if time_str.find("00: 00:") == -1 :                     # Check if duration is 00H: 00M, if yes ignore
                # The duration is less than a minute so ignore it 
            #zt = datetime.datetime(0, 0, 0)
            #if arow[4] == zt:
        #        print "Flight start: ", arow[1], " Reg: ", arow[8], " id: ", arow[9], " 00H: 00M: 00S Duration, delete from db"
        #        cursor.execute('''DELETE FROM flight_log_final WHERE id = ?''', (arow[9],))
        #        continue 
        
        #print "Duration of flight in flight_log2: ", time_str , " edate: ", arow[2]
...
##        window.show()
    ...
    #    print "flight_no is: %d % row[9]
        if row[3] == sar:    # See 20210821 Fix. row[4] is duration which will be blank.  This is an etime="SAR" record so just write to flight_log
            print("SAR flight: ", "Registration: ", row[8], " stime: ", row[1])
            row[4] = ""                                                         # Set duration blank
        else:    
            time_str = row[4].replace("h", "")
            time_str = time_str.replace("m", "")
            time_str = time_str.replace("s", "")
            print("Duration of non-SAR flight in flight_log_final(HH:MM:SS): ", time_str)
            duration = time.strptime(time_str, "%H: %M: %S")
            if time_str.find("00: 00:") != -1 :         # find returns -1 if value not found
                print("Flight start, but zero duration, ignore it: ", row[1], " Reg: ", row[8], " id: ", row[9])
                continue
             
            #print "Standard flight, non-zero duration: ", row[1], " Reg: ", row[8], " id: ", row[9], " Duration: ", time_str
    ...
    #
    # This determines if the take off point was on the airfield, ie within settings.FLOGGER_AIRFIELD_RADIUS km radius.
    #   FLOGGER_AIRFIELD_RADIUS is converted to int on input from gui or changed
    #
...
##
## 20220516 Coords for Vincenty must be reals
##

##        take_off_coords = (data[0], data[1])
##        airfield_coords = (settings.FLOGGER_LATITUDE, settings.FLOGGER_LONGITUDE)
        take_off_coords = (float(data[0]), float(data[1]))
        airfield_coords = (float(settings.FLOGGER_LATITUDE), float(settings.FLOGGER_LONGITUDE))
        print("Flight No: ", row[9], " Takeoff from: ", take_off_coords, " Airfield at: ", airfield_coords)
        dist =  vincenty(take_off_coords, airfield_coords) # Result in kilometres
##
## 20220517 This needs to change to use AirfieldRadius not constant value 2
##

##        if dist > 2:
        if dist > int(settings.FLOGGER_AIRFIELD_RADIUS):
            print("Take off outside radius of airfield location, hence not from here, omit. Radius is: ", int(settings.FLOGGER_AIRFIELD_RADIUS), "ToD is: ", dist)
            continue
        ...
        if strt_date >= max_date:
            ...
            delta_alt = (float(row[6]) - (float(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN)))
            ...
            if duration > MINTIME:
                ...
                print("Row copied")
#            else:
            elif (float(row[6]) - (float(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN))) <= 0:
...
#                if row[6] <= (settings.FLOGGER_QNH + settings.FLOGGER_QFE_MIN):
                print("====Ignore row, flight time too short and too low. Time: ", row[4], " alt: ", row[6], " QNH Min: ", (float(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN)))
#                else:
            elif (float(row[6]) - (float(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN))) > 0:
                print("++++Accept row, short flight but ok min height. Time: ", row[4], " alt: ", row[6], " QNH Min: ", (float(settings.FLOGGER_QNH) + int(settings.FLOGGER_QFE_MIN)))
                cursor.execute('''INSERT INTO flight_log(sdate, stime, edate, etime, duration, src_callsign, max_altitude, speed, registration,flight_no)
                                VALUES(:sdate,:stime,:edate,:etime,:duration,:src_callsign,:max_altitude,:speed, :registration,:flight_no)''',
                                {'sdate':row[0], 'stime':row[1], 'edate': row[2], 'etime':row[3],
                                'duration': row[4], 'src_callsign':row[5], 'max_altitude':row[6], 'speed':row[7], 'registration':row[8], 'flight_no':row[9]})
...
#        print "New method total duration is: ", total_duration
        ...
        if times[1] != "SAR":       # This is a flight group with no end time, ie SAR, don't compute duration
            if times != (None, None):
