...
def landout_check(flight_reg, flight_no, af_centre, radius, landing_coords, mode, settings):
    ...
    #
    af_base = [] 
    af_base = [float(af_centre[0]), float(af_centre[1])]
    print("landout_check called. Registration: ", flight_reg, " Base Airfield coords: ", af_base, " End coords: ", landing_coords)
...
##    landing_dist = vincenty(af_centre, landing_coords)  
    landing_dist = vincenty(af_base, landing_coords)
    print("Landing distance is: %d metres from airfield centre" % landing_dist)
    if landing_dist <= int(radius):
        ...
        result = True
        print("Flight landed out, send msg. Registration: ", flight_reg, " Flight No: ", flight_no, " Distance: ", landing_dist)
    ...
    if result == True:                          # Only send msg if landed out
        #
        ...
        if mode == "email":
            ...
            msg['From'] = fromaddr
            msg['To'] = toaddr
            txt = "%s: %s %s at: %s. Time: %s. Distance: %s" % (settings.APRS_USER, flight_reg, landing_status, landing_coords, now, str(landing_dist))
