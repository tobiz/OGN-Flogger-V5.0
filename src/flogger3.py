...
##from .flogger_inc_flt_cnt import *
from flogger_email_msg import email_msg
from flogger_find_tug import find_tug
from flogger_test_YorN import test_YorN
from flogger_gui import *
...
class flogger3(QtCore.QThread):
    
##    change_status = Signal(QString, QString) 
    ...
    def __init__(self, interval=1, parent=None):
        ...
        super(flogger3, self).__init__(parent)
        #
        # gpp used to prevent multiple none glider Flarm records
        gpp = True
        return
...
#        print "settings.FLOGGER_DB_SCHEMA: ", settings.FLOGGER_DB_SCHEMA
        ...
        def CheckPrev(callsignKey, dataKey, value):
            ...
            if (callsignKey in nprev_vals) == 1:
        ...
        def set_keepalive(sock, after_idle_sec=600, interval_sec=3, max_fails=5):
            ...
            if hasattr(socket, "TCP_KEEPIDLE") and \
                hasattr(socket, "TCP_KEEPALIVE") and \
        ...
        def fleet_check_new(callsign):
            ...
            print("In fleet check new for: ", callsign_trans(callsign), " FLOGGER_FLEET_CHECK is: ", settings.FLOGGER_FLEET_CHECK)
...
#            if test_YorN(settings.FLOGGER_FLEET_CHECK):
                print("Fleet Check not done: ", settings.FLOGGER_FLEET_CHECK)
...
#                if not test_YorN(settings.FLOGGER_FLEET_CHECK):
        ...
        def APRS_connect(settings):
            #
            # See https://pythonprogramminglanguage.com/socket-client/
            # 20220505 - revised APRS_connet()
            #
            ...
            try:
                ...
                sys.exit()
            try:
##                set_keepalive(s, after_idle_sec=60, interval_sec=3, max_fails=5)
                set_keepalive(s, after_idle_sec=settings.FLOGGER_KEEPALIVE_TIME)
            except Exception as e:
                print("setting socket keepalive failed: ", e)
                sys.exit()

            # Connect to remote server
            s.connect((settings.APRS_SERVER_HOST, int(settings.APRS_SERVER_PORT)))
            print("Socket connected Host: ", settings.APRS_SERVER_HOST, " Port: ", int(settings.APRS_SERVER_PORT))
            try:        
        ...
        def APRS_connect_old (settings):
            #    
            #-----------------------------------------------------------------
            # Connect to the APRS server to receive flarm data 
            # 20220505 - No longer used, see APRS_connect()
            #-----------------------------------------------------------------
            #
...
##                set_keepalive(sock, after_idle_sec=60, interval_sec=3, max_fails=5)
                ...
                set_keepalive(sock, after_idle_sec=settings.FLOGGER_KEEPALIVE_TIME)
#                sock.connect((settings.APRS_SERVER_HOST, settings.APRS_SERVER_PORT))
                ...
                print("Socket failure on connect: ", e, " Host: ", settings.APRS_SERVER_HOST, " Port: ", int(settings.APRS_SERVER_PORT))
                result = dns.resolver.query(settings.APRS_SERVER_HOST, 'A')
                for ipval in result:
                    print(('IP', ipval.to_text()))
                    try:
                        sock.connect((ipval.to_text(), int(settings.APRS_SERVER_PORT)))
                    except Exception as e:
                        print(("2nd try socket connect failure: ", ipval.to_text(), " Port: ", int(settings.APRS_SERVER_PORT)))
                        print ("We're done")
                exit()
...
##
## string find function changed python3
##

##                if string.find(str(packet_str), base) != -1:
                if str(packet_str).find(base) != -1:
...
##
## Python3 string find function has changed
##

##                file_find = string.find(full_file, filename) != -1
                file_find = full_file.find(filename) != -1
...
##                sock.shutdown(0)
                ...
                print("Attribute error: module' object has no attribute 'shutdown: ", error)
##            except socket.error as e:
            except OSError as e:
##                if 'not connected' in e:
##                    print('*** Transport endpoint is not connected ***')
                print("socket no longer open so can't be closed, create new one: ", e)    
            else:
...
##            set_keepalive(sock, after_idle_sec=60, interval_sec=3, max_fails=5)
##
## 20220513 Is this needed? Or just socket.send("#OGNFLOG5 App KeepAlive:") or similar?
##
##            set_keepalive(sock, after_idle_sec=settings.FLOGGER_KEEPALIVE_TIME)
            try:
                sock.connect((settings.APRS_SERVER_HOST, int(settings.APRS_SERVER_PORT)))
        ...
        #            sock.send('user %s pass %s vers Python_Example 0.0.1 filter r/+54.228833/-1.209639/25\n ' % (settings.APRS_USER, settings.APRS_PASSCODE))
            try:
                sock.send(encode.APRSparm())
                # Make the connection to the server
                sock_file = sock.makefile()
                print("sock_file = sock.makefile()")
                exit()
                return sock_file
            except:
                print("Sock.send Failure")
                exit()
...
#            print "Parsing cmd line args failed"
        ...
        try:
            ...
            print("Cmd Line args parsed ok ", type(args))
            #
            ...
            #if args.ver <> None:
            #    print "Taken from args.version: ", settings.FLOGGER_VER
            #    settings.FLOGGER_VER = args.ver 
            #else:
            #    print "Taken from FLOGGER_Ver: ", settings.FLOGGER_VER            
        except :
...
#       location.horizon = settings.FLOGGER_LOCATION_HORIZON    # Adjustments for angle to horizon
        ...
        location.elevation = float(settings.FLOGGER_QNH)
        ...
        locationAF = [str(settings.FLOGGER_LATITUDE), str(settings.FLOGGER_LONGITUDE), settings.FLOGGER_QNH]
...
#        location.date = ephem.Date(log_datetime)
        ...
        dtd = datetime.datetime.strptime(str(settings.FLOGGER_MIN_FLIGHT_TIME), "%H:%M:%S")
        dtb = datetime.datetime.strptime("0:0:0", "%H:%M:%S")   
        min_flight_time = dtd - dtb  # min_flight_time is of type timedelta
        print("Minimum flight time is: ", min_flight_time)
...
#        print "Libfap return: ", rtn
        ...
        if settings.FLOGGER_LOG_PATH != "":
            if not os.path.isdir(settings.FLOGGER_LOG_PATH):
                print("Log path is not directory", end=' ') 
...
#        sig_handler(db, cursor)
        ...
        locationAF = [str(settings.FLOGGER_LATITUDE), str(settings.FLOGGER_LONGITUDE), float(settings.FLOGGER_QNH)]
        check_sunset = init_sunset_test(locationAF)
        ...
        #
        print("Test signals")
        self.change_status.emit("Processing", "green")
        print("Signal set_flight_count")
        self.set_flight_count.emit("0", "None", "None")
##        self.set_flight_count.emit("0")
        print("Display sunset time")
        check_sunset.is_sunset_now(locationAF)
        self.set_sunset.emit(check_sunset.local_sunset())
...
##                print("Check Sunset")
                if not check_sunset.is_sunset_now(locationAF):
                    print("Sun not set")
                    pass
                    #print "Sun not set"
...
##DEL                    delete_table("flights")
                    ...
                    # Note this looks wrong as it deosn't take into account
                    # the Timezone which is dependent on the location.
                    # Needs some thought
                    #
...
#                    self.RunningLabel.setText("Sleeping")
                    self.change_status.emit("Waiting for sunrise", "yellow")
                ...
                if (current_time - keepalive_time) > float(settings.FLOGGER_KEEPALIVE_TIME):
                    try:
                        print("Socket open for: ", (current_time - keepalive_time), " seconds, send keepalive")
                        keepalive_str = "#OGNFLOG5 App KeepAlive:\n\n"
                        rtn = sock.send(keepalive_str.encode())
                        print("OGNFLOG5 - Send keepalive", elapsed_time, " rtn is: ", rtn)
                        keepalive_time = current_time
                    except Exception as e:
                        print(('something\'s wrong with socket send. Exception type is %s' % (repr(e))))
        ...
        #                datafile.write(packet_str)
                    ...
                    print("Socket error on readline @ line 1221. Error: ", e)
                    try:
                        sock_file = connect_APRS(socket)
                    except socket.error as e1:
                        print("Socket error after close and open new socket, give up. Error: ", e1, " Socket: ", socket)
                        sys.exit()
                    packet_str = sock_file.readline()
                    print("Raw APRS Packet (Re-read after socket failure): ", packet_str)
...
#                print "packet string length is: ", len(packet_str), " packet is: ", packet_str
                ...
                # See http://wiki.glidernet.org/wiki:subscribe-to-ogn-data for handshake
                if len_packet_str == 0:
        ...
        #            continue
#
# 20210827 Try to fix failure to connect to new socket after zero length packet returned
#                    
                    try:
##                        sock.close()
                        sock_file = connect_APRS(socket)
                    except socket.error as e1:
                        print("20210827. Socket error after close and open new socket, give up. Error: ", e1, " Socket: ", socket)
##                        sys.exit()
                    continue
#
# End
...
##                    set_keepalive(sock, after_idle_sec=settings.FLOGGER_KEEPALIVE_TIME)
                    try:
                        ...
                        print("Connection refused. Errno: ", e, " SERVER_HOST: ", settings.APRS_SERVER_HOST, " APRS_SERVER_PORT: ", int(settings.APRS_SERVER_PORT))
                        exit() 
                    APRSparm = ('user %s pass %s vers %s %s filter r/%s/%s/%s\n ' % (settings.APRS_USER, 
                                                                                     settings.APRS_PASSCODE, 
                                                                                     settings.FLOGGER_NAME, 
        ...
        #            exit()
                ...
                if packet:
                    print("aprs_parse rtnd Ok: ", packet)
                    gpp = True
                else:
##                    print("aprs_parse rtnd Error: ", packet)
##                    print ("aprs_parse Failed. Not glider position packet")
                    
                    gpp = False
##                    print("-----------------End of Packet: ", i, " Not glider position packet ------------------------------")
                    continue
                ...
                print("src_callsign from: ", src_callsign)   
                #
...
#                    print "Aircraft ", src_callsign, " not registered at ", settings.FLOGGER_AIRFIELD_NAME, " , ignore"
                    print("-----------------End of Packet: ", i, " Not in FlarmDB ------------------------------")
                    continue
                else:
...
#                    fl_date_time = local_time.strftime("%D:%H:%M:%S")
        ...
        #            takeoff_loc = (latitude, longitude)
                    self.base_coords = (float(settings.FLOGGER_LATITUDE), float(settings.FLOGGER_LONGITUDE))
                    print("base_coords", self.base_coords)
                    self.new_coords = (latitude, longitude)
                                        
##                    takeoff_dist_1 = vincenty(self.base_coords, self.new_coords)                    
                    takeoff_dist = vincenty(self.base_coords, self.new_coords)
##                    takeoff_dist = takeoff_dist_1.meters
                    print("Take off distance is: ", takeoff_dist)                    
##                    takeoff_dist = vincenty(self.base_coords, self.new_coords).kilometers
##
## Temporary remove to reduce log
##

##                    print("Takeoff distance for: ", registration, " is: ", takeoff_dist, "metres", \
##                            " Base Lat: ", settings.FLOGGER_LATITUDE, \
##                            " Base Lon: ", settings.FLOGGER_LONGITUDE, \
##                            " Actl Lat: ", latitude, \
##                            " Actl Lon: ", longitude)
                    print("Test for was stopped now moving. nprevs[speed] is: " + str(nprev_vals[src_callsign]['speed']) + " nvalues[speed] is: "+ str(nvalues[src_callsign]['speed']) + " Takeoff Distance: ", takeoff_dist)
        ...
        #            if nprev_vals[src_callsign]['speed'] <= V_SMALL and nvalues[src_callsign]['speed'] > V_SMALL and takeoff_dist < settings.FLOGGER_AIRFIELD_LIMIT:
                    if nprev_vals[src_callsign]['speed'] <= V_SMALL \
                        and nvalues[src_callsign]['speed'] > V_TAKEOFF_MIN \
                        and takeoff_dist <= int(settings.FLOGGER_AIRFIELD_RADIUS):
##                        and takeoff_dist < int(settings.FLOGGER_AIRFIELD_LIMIT):
...
##                        print("New test true for switch-on")
                        print("***Takeoff inside Airfield radius: ", takeoff_dist, " Registration: ", registration, " Start Time: ", fl_time)

##
## Temporary remove: email not setup on test sysyem
##
...
##                        email_msg(settings.FLOGGER_SMTP_TX, settings.FLOGGER_SMTP_RX, registration, fl_time, settings)
                    ...
                    # 20210821 Allow for flight never lands situation - test
                    #                "SAR", Search and Rescue will show in edate, etime if flight never lands, eg Flarm fails, switched off etc
        ...
        #                fl_end_time = fl_end_time_str
                        ...
                        fl_duration_datetime = fl_end_time - fl_start_time  # fl_duration_time is type timedelta a string format %H:%M:%S
        ...
        #                fl_duration_time = "%.2dh: %.2dm: %.2ds" % (c.seconds//3600,(c.seconds//60)%60, c.seconds%60)
                        ...
                        #self.set_flight_count.emit(str(flight))   # Update GUI for this flight logged to db
                        ...
                        print("Updated flight_log_final", src_callsign, " For registration: ", registration)
        ...
        #
                        ...
                        #
                        # Check if this flight duration is >= minimum flight time.
                        # If yes increment the number of flights and output this on the GUI
                        #
                        if fl_duration_datetime >= min_flight_time:
                            flight_count = flight_count + 1
##                            self.set_flight_count.emit(str(flight_count), str(registration), str(fl_end_time_str))
                            self.set_flight_count.emit(int(flight_count), str(registration), str(fl_end_time_str))
                        ...
                        print ("-----------------End of Packet: ", i, " Increment GUI count for: ", str(registration), " ------------------------------")
                        continue
        #            if nprev_vals[src_callsign]['speed'] == 0 and nvalues[src_callsign]['speed'] == 0:
        ...
        #            if nprev_vals[src_callsign]['speed'] <= V_SMALL and nvalues[src_callsign]['speed'] <= V_SMALL and nvalues[src_callsign]['altitude'] <= settings.FLOGGER_QNH:
                    if nprev_vals[src_callsign]['speed'] <= V_TAKEOFF_MIN \
                        and nvalues[src_callsign]['speed'] <= V_TAKEOFF_MIN \
                        and nvalues[src_callsign]['altitude'] <= settings.FLOGGER_QNH:
                        # Aircraft hasn't moved and is not at an altitude greater than Airfield QNH.  
                        ...
                        #
                        # 20210923 Temporary fix to cater for takeoff where new alt is lower than previous but current speed greater than  TAKEOFF_MIN and previous speed <= TAKEOFF_MIN
                        #
                        #if nvalues[src_callsign]['altitude'] > nprev_vals[src_callsign]['altitude']:
                        if (nvalues[src_callsign]['altitude'] > nprev_vals[src_callsign]['altitude']) or (nprev_vals[src_callsign]['speed'] <= V_TAKEOFF_MIN and nvalues[src_callsign]['speed'] > V_TAKEOFF_MIN):
                            print("20210923. Aircraft ", src_callsign, " is now higher than max height, was: ", nprev_vals[src_callsign]['altitude'], " now: ", nvalues[src_callsign]['altitude'])
                            ...
                            nprev_vals[src_callsign]['speed'] = nvalues[src_callsign]['speed']      
##                            print ("Callsign: ", src_callsign, " Not higher ")
##
## 20220613 Is this continue statement needed or does the db.commit() need to be doen?
##
##                            continue
                        
                    print("Values for callsign Commit: ", src_callsign, " Values are: ", nvalues[src_callsign], " Prev_vals are: ", nprev_vals[src_callsign])
                    db.commit()        
                print ("-----------------End of Packet: ", i, " Processed ------------------------------")
