...
##from .flogger_release_height import *
from open_db import opendb 
from flogger_test_YorN import test_YorN
from flogger_release_height import *
...
#                    print "Tug Fleet check. Registration: ", registration, " Type: ",  aircraft_type[0]
    ...
    cursor.execute('''SELECT id, stime, duration, registration, max_altitude, flight_no FROM flights ORDER by stime''')
...
#                print "Next row candidate for a glider is: ", flight
                ...
                if glider_fleet_check(flight[3], settings):
                    ...
                    print("Delta tug and glider take-off time is: ", tdelta_sec, " Glider reg: ", flight[3], " Tug reg: ", row[3])
            ...
            try:
                # Row[4] is tug flight record id from initial select
                #relH = release_height(cursor, tug_flight_id, flight_id, settings)
                relH = release_height(cursor, tug_flight_id, flight[5], settings)
                print("Release height: ", relH)
            except:
                # Failed to compute, use old method
                print("Release_height failed with Tug flight_no: ", tug_flight_id, " Glider flight_no: ", flight[5])
