...
#import os
#import string
from PyQt5.QtWidgets import QFileDialog, QApplication, QWidget, QPushButton, QVBoxLayout, QDialog
...
import PyQt5
##from src.flogger_process_log import process_log
...
##    pyrcc4_cmd = "pyrcc4 -py3 -o %s %s" % (pyrcc4_out, pyrcc4_in)
    pyrcc4_cmd = "pyrcc5 -o %s %s" % (pyrcc4_out, pyrcc4_in)
...
##    print("pyrcc4 ran ok")
    ...
    print("PyQt5 flogger_resources_rc.py built and imported-1")
except:
...
#Ui_MainWindow, base_class = uic.loadUiType(path_join_dd(os.path.abspath(__file__), ["data", "flogger-v1.2.1.ui"]))
#Ui_MainWindow, base_class = uic.loadUiType(path_join_dd(os.path.abspath(__file__), ["data", "flogger-v1.2.2.ui"]))

print ("loadUiType parm: ", path_join_dd(os.path.abspath(__file__), ["data", "flogger-v1.2.3.ui"]))
##Ui_MainWindow, base_class = uic.loadUiType(path_join_dd(os.path.abspath(__file__), ["data", "flogger-v1.2.3.ui"]))
...
#      
    ...
    def set_flight_count(self, number, flightReg, flightEndTime):
        print("set_flight_count called: ", number, " Registration: ", flightReg, " Flight end time: ", flightEndTime)
        self.FlightCount.display(int(number))
        self.LastFlightReg.setText(str(flightReg))
        self.FlightEndTime.setText(str(flightEndTime))
...
#            if row[9] is None: 
            ...
            if row[9] is None and row[10] is None and row[11] is None:
                ...
                val = row[9]
            self.ui.FlightLogTable.setItem(rowPosition , 0, QtGui.QTableWidgetItem(val))           # Tug Reg
            if row[11] is None:
                val = "----"
            else:
                val = row[11]
            self.ui.FlightLogTable.setItem(rowPosition , 1, QtGui.QTableWidgetItem(val))           # Tug Type
            self.ui.FlightLogTable.setItem(rowPosition , 2, QtGui.QTableWidgetItem(row[7]))        # (Moto) Glider     
            self.ui.FlightLogTable.setItem(rowPosition , 3, QtGui.QTableWidgetItem(row[7][3:]))    # CN
            ...
            self.ui.FlightLogTable.setItem(rowPosition , 4, QtGui.QTableWidgetItem(plane_type[0])) # Plane Type/Model 
            self.ui.FlightLogTable.setItem(rowPosition , 5, QtGui.QTableWidgetItem(row[2]))        # Glider Takeoff TIme
            self.ui.FlightLogTable.setItem(rowPosition , 6, QtGui.QTableWidgetItem(row[3]))        # Glider Landing Time
            self.ui.FlightLogTable.setItem(rowPosition , 7, QtGui.QTableWidgetItem(row[4]))        # Glider Flight Time
            if row[10] is None:
                val = "----"
            else:
                val = row[10]
            self.ui.FlightLogTable.setItem(rowPosition , 8, QtGui.QTableWidgetItem(val))            # Tug Max ALt (QFE)
            self.ui.FlightLogTable.setItem(rowPosition , 9, QtGui.QTableWidgetItem(row[6]))
            self.ui.FlightLogTable.setItem(rowPosition , 10, QtGui.QTableWidgetItem(row[8]))
